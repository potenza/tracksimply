...
class Track::VisitsControllerTest < ActionController::TestCase
  test "#new creates a visit record" do
    assert_difference("Visit.count") do
      get :new, token: tracking_links(:paid_search).token
    end
    assert_equal tracking_links(:paid_search).id, assigns(:visit).tracking_link_id
  ...
  test "#new redirects to the tracking link landing page" do
    get :new, token: tracking_links(:paid_search).token
    assert_redirected_to tracking_links(:paid_search).landing_page_url
  end
end
